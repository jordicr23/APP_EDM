...
ui = navbarPage(
    ...
    div(
      ...
      absolutePanel(
        ...
        fluidRow(column(width = 6,
                        uiOutput("selecall")), column(width = 6,
                                                      uiOutput("selecnone")))
      ...
      dashboardBody(fluidRow(column(
        ...
        fluidRow(
          box(h3(
            strong("Visualización base de datos"),
            align =
              "center"
          ),
          width =
        ...
        column(width = 7,
               ...
               fluidRow(
                 box(
                   width = 10,
                   align = "center",
                   withSpinner(plotlyOutput("curvaROC"), type = 4)
                 )
               ))
      ))
    )
  ...
  tabPanel(
    title = "MEMORIA",
    icon = icon("info"),
    div(class = "inner", dashboardBody(fluidRow(
      column(
        width = 10,
        offset = 1,
        align = "justify",
        ...
        tags$img(
          src = 'https://raw.githubusercontent.com/jordicr23/APP_EDM/main/boceto.png',
          deleteFile = F,
          width = "100%",
          height = "100%",
        ...
        HTML(
          "<font color='black' size=5 text-align= 'justify'> <b> 2. Idea de negocio </b><br><br></font>"
        ),
        HTML(
          "<font color='black' size=4 text-align= 'justify'>Hoy en día cada vez más empresas basan su capacidad de llegada a los posibles clientes en la aplicación de métodos de clasificación de estos en distintos grupos. Nuestra idea de negocio es proporcionar a grandes empresas una interfaz gráfica que les permita clasificar a los clientes sin la necesidad de utilizar alguna herramienta compleja en dos grupos: los clientes que pueden recibir una campaña de marketing favorablemente y los que no. De este modo, la organización de la empresa puede decidirse a enfocar sus esfuerzos de marketing y así evitar malgastar sus recursos tanto físicos como económicos. Con nuestra herramienta informaríamos a la empresa cuál es el coste que se ahorrarían respecto a la estrategia que siguen en la actualidad, y nosotros como fundadores de esta utilidad mantendriamos la herramienta en constante evolución para satisfacer las demandas variables de nuestros clientes y cobraríamos un 25% del ahorro de la empresa por cambiar de política.<br></font>"
        ),
        HTML(
          "<font color='black' size=5 text-align= 'justify'><br> <b> 3. Ejemplo de funcionamiento </b><br><br></font>"
        ),
        HTML(
          "<font color='black' size=4 text-align= 'justify'>Una aplicación tipo <i>dashboard</i> es una herramienta muy útil, hemos intentado que sea lo más aprovechable posible permitiendo al usuario cargar diferentes tipos de archivos (.csv y .arff), elegir con qué variables va a trabajar y que modelos va a aplicar entre otras muchas funcionalidades aunque de cara a la aplicación de esta herramienta se podrían añadir otras operaciones.<br>
Puesto que no sabemos si hemos sido capaces de abordar todos los posibles problemas que podrían generarse al utilizar distintos ficheros de datos a continuación mostramos una pequeña visualización de cómo se comportaría nuestro <i>dashboard</i>:<br><br>
        ...
        tags$iframe(
          width = "100%",
          height = "550px",
          src = "https://www.youtube.com/embed/1nxj8bWFe38",
          frameborder = "0",
          allow = "accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture",
          allowfullscreen = NA
        ),
        HTML(
          "<font color='black' size=4 text-align= 'justify'><br><br>Proporcionamos algunos ficheros con los que hemos trabajado por si se desea recrear esta demostración:<br></font>"
        ),
        downloadLink("credit", "German Credit data (credit.csv)"),
        HTML(
          "<font color='black' size=3 text-align= 'justify'>:
...
Este conjunto de datos multivariante fue introducido por Ronald Fisher. El fichero original dispone de información respecto a tres tipos de flores, nosotros hemos utilizado solo dos de las variedades (Clasificar según la variable <i>class</i>)<br></font>"
        ...
        HTML(
          "<font color='black' size=3 text-align= 'justify'>: Datos de los estudiantes de secundaria relacionados con la decisión de ir a la universidad. Estos datos pretenden predecir si los estudiantes continuarán o no con la universidad. (Clasificar según la variable <i>in_college</i>)<br></font>"
        ),
        downloadLink(
          "marketing",
          "Kaggle::Marketing Campaign Dataset (marketing.csv)"
        ),
        HTML(
          "<font color='black' size=3 text-align= 'justify'>: Conjunto de datos sobre campañas de marketing. El objetivo es predecir quién responderá a una oferta de un producto o servicio. (Clasificar según la variable <i>Response</i>)<br></font>"
        ),
        HTML(
          "<font color='black' size=3 text-align= 'center'><br><br> Trabajo realizado por Jordi Caravaca Rostoll y Pablo Riera Carda para la asignatura Evaluación y Despliege de Modelos. <br><br></font>"
        ),
...
server <- function(input, output, session) {
  ...
  output$checkbox <- renderUI({
    multiInput(
      inputId = "select_var",
      label = HTML(
        "<font color='black' size=3>Seleccione las variables a estudiar: </font>"
      ),
      choices = names(df()),
      options = list(
        enable_search = FALSE,
  ...
  modelo = eventReactive(input$calcular, {
    ...
    if ((sort(unique(df_model[, input$var_pred]))[1] == 0) &
        (sort(unique(df_model[, input$var_pred]))[2] == 1)) {
  ...
  output$iris <- downloadHandler(
    ...
    content = function(con) {
      data = read.arff(url(
        "https://raw.githubusercontent.com/jordicr23/APP_EDM/main/iris.arff"
      ))
      write.arff(data, con)
    }
  )
